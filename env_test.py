...
while True:
  ...
  for act_human in acts_human:
    #act_human.function: _Functions.Smart_screen
    #act_human.arguments: [[<Queued.now: 0>], [49, 27]]
    ...
    #action_1[0].function: _Functions.no_op
    #action_1[0].arguments: [[0], (104, 98)]
    agent_function = str(action_1[0].function)
    agent_arguments = str(action_1[0].arguments)
    agent_action_name = agent_function.split('.')[-1]
    agent_action_index = action_type_list.index(actions._Functions[agent_action_name])
    print("agent_action_index: " + str(agent_action_index))
  
  #actions.FunctionCall(_SELECT_POINT, [_NOT_QUEUED, [0,0]])

  #act_human: [FunctionCall(function=<_Functions.Build_Refinery_screen: 79>, arguments=[[<Queued.now: 0>], [104, 38]]), FunctionCall(function=<_Functions.select_control_group: 4>, arguments=[[<ControlGroupAct.recall: 0>], [2]])]
  #action_1: [FunctionCall(function=<_Functions.no_op: 0>, arguments=[])]
